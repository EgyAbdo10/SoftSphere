...
class FileStorage:
    ...
    def all(self, cls=None):
        ...
        cls: the class itself not its name
        """
        new_dict = {}

        if cls in self.cls_names.values():
            for key, val in FileStorage.__objects.items():
                if val.__class__ == cls:
                    new_dict[key] = val.to_dict()

        elif cls in self.cls_names.keys():
            for key, val in FileStorage.__objects.items():
                ...
                with open(FileStorage.__file_path, "r") as f:
                    ...
                    for key, obj_dict in data_dict.items():
                        ...
                        FileStorage.__objects[key] = (
                            self.cls_names[cls_name](**obj_dict)
                        )
            except:
                pass
        else:
    ...
    def find(self, id):
        ...
        objs = FileStorage.__objects.values()
        for obj in objs:
            if obj.id == id:
                return obj
        return None